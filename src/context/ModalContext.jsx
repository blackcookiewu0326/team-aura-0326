...
export const ModalProvider = ({ children }) => {
   const { actions, categories, roles } = useGlobalData();
   const { showToast } = useToast();


   // --- State ---
   const [taskModal, setTaskModal] = useState({ isOpen: false, id: null, data: { title: '', points: 10, icon: '🐾', description: '', week: '1', type: 'fixed', category: '一般', categoryId: null, isPinned: false } });
   ...
   const openTaskModal = (task = null) => {
       if (task) {
           setTaskModal({ isOpen: true, id: task.id, firestoreId: task.firestoreId, data: { ...task, categoryId: task.categoryId || null } });
           ...
           setTaskModal({ isOpen: true, id: null, firestoreId: null, data: { title: '', points: 10, icon: '🐾', description: '', week: '1', type: 'fixed', category: '一般', categoryId: null, isPinned: false } });
